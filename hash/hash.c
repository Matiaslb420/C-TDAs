...
long hash_encontrar(const hash_t* hash, const char* clave, int hashing){
    char* clave_dup = strdup(clave);
    long posicion = (long)hashear(clave_dup,hash->tamano_actual, hashing);
    free(clave_dup);
    return posicion;
}
...
hash_t *hash_crear(){
    ...
    hash->arreglo = (size_t*)malloc(TAMANO_ARREGLO * sizeof(size_t));
